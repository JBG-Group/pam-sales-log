...
- **複数フォルダ・共有ドライブ対応**：置き場所を移したあとも旧フォルダを並行して見られる。処理済みは各フォルダ自身の配下へ移動
...
| `DRIVE_LOG_FOLDER_IDS` | 音声を置くDriveフォルダのID。**カンマ区切りで複数指定可**（上から順に走査） |
...
Drive API は既定ではマイドライブしか検索しません。共有ドライブ上のフォルダを扱うには `supportsAllDrives=True` と `includeItemsFromAllDrives=True` が必要です。本ツールは全APIに付与済みです。

それでも見えない場合は、認証したGoogleアカウントがその共有ドライブのメンバーか確認してください。権限がないと `404` になります（本ツールはそのフォルダだけ警告してスキップし、他のフォルダは処理を続けます）。

### 置き場所を移したが、旧フォルダに入れる人もいる

`DRIVE_LOG_FOLDER_IDS` に新旧両方を並べれば、どちらに置かれても拾えます。処理済みファイルはそれぞれのフォルダ配下の処理済みフォルダへ移るので、共有ドライブとマイドライブの間でファイルが行き来することはありません。

```
DRIVE_LOG_FOLDER_IDS=新フォルダID,旧フォルダID
```
